...
$if data entity @s Inventory[{Slot:$(cint)b}].components."minecraft:custom_data".ulg.BackPack{Opened:1b} run data modify storage ulg:macro packet.string set value "opened"
$if data entity @s Inventory[{Slot:$(cint)b}].components."minecraft:custom_data".ulg.BackPack{Opened:1b} run data modify storage ulg:macro packet.opened set value "1"
